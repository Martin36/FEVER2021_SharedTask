...
def calculate_score(data, related_sents, k, print_low_recall=False):
    sum_precision = 0
    sum_recall = 0
    counter = 0
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        type=int,
        help="The number of retrieved sentences for each claim",
    )
    ...
    if ".jsonl" not in args.top_k_sents_path:
        raise RuntimeError(
            "The top k docs sents should include the name of the .json file"
        )
    if not args.k:
    ...
    precision, recall = calculate_score(data, related_sents, args.k)
