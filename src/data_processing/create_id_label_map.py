...
def main():
    ...
    parser.add_argument("--train_data_file", default=None, type=str, help="Path to the FEVEROUS train data file e.g. train.jsonl")
    parser.add_argument("--dev_data_file", default=None, type=str, help="Path to the FEVEROUS dev data file e.g. dev.jsonl")
    parser.add_argument("--train_out_file", default=None, type=str, help="Path to the train output file, should be .json format")
    parser.add_argument("--dev_out_file", default=None, type=str, help="Path to the dev output file, should be .json format")
    ...
    if ".json" not in args.train_out_file:
        raise RuntimeError("The train out data file path should include the name of the .json file")
    if not args.dev_out_file:
        raise RuntimeError("Invalid dev out data file path")
    if ".json" not in args.dev_out_file:
        raise RuntimeError("The dev out data file path should include the name of the .json file")
