...
def main():
    ...
    parser.add_argument("--top_tables_file", default=None, type=str, help="Path to the csv file containing the tapas data")
    parser.add_argument("--top_sentences_file", default=None, type=str, help="Path to the jsonl file containing the sentence evidence")
    parser.add_argument("--id_label_map_file", default=None, type=str, help="Path to the json file containing the id label mapping")
    parser.add_argument("--out_file", default=None, type=str, help="Path to the output csv file to store the merged data")
    ...
    if ".jsonl" not in args.top_tables_file:
        raise RuntimeError("The tapas csv file path should include the name of the .csv file")
    if not args.top_sentences_file:
        raise RuntimeError("Invalid sentence data file path")
    if ".jsonl" not in args.top_sentences_file:
        raise RuntimeError("The sentence data file path should include the name of the .jsonl file")
    if not args.id_label_map_file:
        raise RuntimeError("Invalid id label map file path")
    if ".json" not in args.id_label_map_file:
        raise RuntimeError("The id label map file path should include the name of the .jsonl file")
    if not args.out_file:
        raise RuntimeError("Invalid output file path")
    if ".csv" not in args.out_file:
        raise RuntimeError("The output file path should include the name of the .csv file")
